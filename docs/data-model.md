...
The database is `workout-conductor-v4`, version 5, opened by `src/core/storage/indexedDb.ts`. Upgrades only add stores.
...
Version 2 added the three custom-content stores and version 5 the `outbox` and `cloud` stores;
...
Real profiles, exports, and history, and the cloud copy's token. The repository holds only source
