...
the generator): isolation exercises first, the one whose least-trained muscle already gets the most
work from the main lifts still in the plan going first (`mainLiftSets`: a main lift's set counts in
full for its primary muscles, at half for its secondary ones, and nothing for the hamstrings in a
squat or a hip thrust); the day's only work for a muscle last among them, core work always counting
as that; then lower-back work (only an exercise the lifter made can be one: the catalog's only lift
with the lower back as a primary muscle is the deadlift, a main lift); and a main (multi-joint) lift
only once no isolation row is left. A row ranks as its best-kept exercise. At equal work the lowest
value goes first, where paired rows are worth more than their weakest member because they save time.
A fresh plan at a default length of 60 or 75 minutes fits every exercise in all 600 the report
