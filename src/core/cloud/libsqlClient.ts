import { CLOUD_URL, type CloudClient, type CloudStatement } from './model';
 ...
 */
export async function createLibsqlCloudClient(token: string): Promise<CloudClient> {
  const { createClient } = await import('@libsql/client/web');
  const client = createClient({ url: CLOUD_URL, authToken: token });
